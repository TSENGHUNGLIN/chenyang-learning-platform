...
### CSV 預覽改進
- [x] 檢查現有 CSV 預覽功能（CSVPreviewDialog 和 LocalCSVPreviewDialog）
- [x] 新增 CSV 預覽分頁優化（超過 100 行顯示提示）
- [x] 改進 CSV 預覽 UI（更好的表格樣式、欄位寬度調整）
- [ ] 新增 CSV 格式驗證和錯誤提示
- [ ] 新增 CSV 編碼自動偵測功能

### 考卷範本預覽功能
- [x] 建立考卷範本預覽對話框元件
- [x] 顯示範本基本資訊（名稱、描述、題目數量、總分）
- [x] 顯示範本題目列表（題型、難度、分數）
- [x] 新增「使用此範本建立考試」快速按鈕
- [x] 新增範本匯出功能（PDF/Word）（按鈕已建立，實際匯出功能待開發）

### 檔案預覽改進
- [x] 檢查現有檔案預覽功能（FilePreviewDialog）
- [x] 改進 DOCX 預覽顯示（保留格式、段落）
- [x] 新增圖片預覽功能（支援放大、縮小、旋轉）
- [x] 新增預覽載入進度指示器

### 題目預覽改進
- [x] 在題庫管理頁面新增題目預覽功能
- [x] 顯示題目完整內容（題目、選項、答案、解釋）
- [x] 新增題目預覽對話框（類似考試作答介面）

### 考試預覽功能
- [x] 在考試管理頁面新增考試預覽功能
- [x] 顯示考試完整資訊（題目列表、分數配置）
- [x] 模擬考生視角預覽考試介面
...
## 新任務：在首頁和檔案管理頁面加入支援檔案格式說明（2025-01-22）

- [x] 在首頁「檔案管理」卡片內加入支援格式說明（DOCX、CSV）
- [x] 在檔案管理頁面卡片內加入支援格式說明
- [x] 測試視覺效果並儲存檢查點
...
- [x] 將蘋果風格卡片設計應用到檔案管理頁面
- [x] 將蘋果風格卡片設計應用到題庫管理頁面
- [x] 將蘋果風格卡片設計應用到考試管理頁面
- [x] 將蘋果風格卡片設計應用到部門人員管理頁面
- [x] 將蘋果風格卡片設計應用到使用者管理頁面
- [x] 將蘋果風格卡片設計應用到AI分析頁面
- [x] 測試所有頁面的視覺效果
...
## 檔案上傳作者判定邏輯修復（2025-01-20）

- [x] 分析檔案上傳時從檔名提取作者名稱的邏輯
- [x] 修復同一作者無法上傳多個檔案的問題
- [x] 修復檔名解析邏輯（任意三個字或四個字的問題）
- [x] 測試檔案上傳功能（同一作者多個檔案、不同檔名格式）




## 檔案上傳優化功能開發（2025-01-20）

### 功能一：檔案歸屬批次修改
- [x] 在檔案管理頁面新增批次選擇功能（複選框）
- [x] 實作批次修改人員歸屬的後端 API
- [x] 實作批次修改 UI（選擇人員對話框）
- [x] 測試批次修改功能
...
## 檔案管理頁面優化功能（2025-01-22）

- [x] 實作搜尋關鍵字歷史記錄功能（顯示在搜尋欄位下方）
- [x] 實作關鍵字黃色高亮顯示功能（已存在）
- [x] 實作檔案列表分頁功能（每頁顯示10個檔案）
- [x] 測試三個功能正常運作




## 修復考試管理頁面 desc is not defined 錯誤（2025-01-22）

- [x] 找出缺少 desc import 的後端 API（db.ts 檔案）
- [x] 新增 desc 函數的 import 語句
- [x] 測試考試管理頁面正常運作


## 優化檔案管理搜尋功能，支援部門和人員姓名搜尋（2025-01-22）

- [x] 檢查現有搜尋功能實作（db.ts 的 getFilesWithFilters 函數）
- [x] 確認目前已支援部門和人員姓名搜尋（後端已實作）
- [x] 更新前端UI，在搜尋結果中高亮顯示部門名稱和人員姓名
- [x] 測試搜尋功能（搜尋部門名稱、人員姓名、檔案內容）


## UI優化：搜尋歷史、卡片設計、首頁活動報告（2025-01-22）

- [x] 修正搜尋歷史記錄，過濾不完整字元（注音）和不完整字句
- [x] 為所有頁面的卡片加入蘋果風格顏色設計
- [x] 優化首頁最新活動報告，限制顯示10條並加入右側滑桿
- [x] 測試所有UI優化功能




## UI優化：蘋果風格側邊欄和檔案上傳對話框（2025-01-22）

- [x] 分析當前側邊欄和檔案上傳對話框的設計
- [x] 優化側邊欄底色，套用蘋果風格漸層或純色背景
...
## CSV 檔案預覽功能開發（2025-01-22）

### 階段一：規劃 CSV 預覽功能架構
- [x] 分析現有檔案管理和考試範本匯入流程
- [x] 設計 CSV 預覽 UI 佈局（表格顯示、分頁、欄位資訊）
- [x] 規劃後端 API 架構（CSV 解析、資料驗證、預覽資料格式）

### 階段二：實作後端 CSV 解析和預覽 API
- [x] 建立 csvPreview.ts 模組
- [x] 實作 CSV 檔案解析功能（支援 UTF-8、BOM 處理）
- [x] 實作預覽資料 API（返回前 100 行資料）
- [x] 實作欄位資訊 API（欄位名稱、資料類型、範例值）
- [x] 新增錯誤處理（格式錯誤、編碼錯誤）

### 階段三：實作檔案管理頁面的 CSV 預覽 UI
- [x] 在檔案列表中為 CSV 檔案新增「CSV 預覽」按鈕
- [x] 建立 CSVPreviewDialog 元件
- [x] 實作表格顯示（使用 shadcn/ui Table 元件）
- [x] 實作分頁功能（每頁顯示 20 行）
- [x] 顯示欄位資訊（欄位數量、總行數）
- [x] 新增下載按鈕（從預覽對話框直接下載）

### 階段四：實作考試範本匯入的 CSV 預覽 UI
- [x] 在考試範本頁面新增 CSV 匯入按鈕
- [x] 建立 LocalCSVPreviewDialog 元件（支援本地檔案預覽）
- [x] 上傳 CSV 後可以點擊預覽按鈕
- [x] 顯示 CSV 格式說明（必要欄位、格式範例）
- [x] 實作表格顯示和分頁功能

### 階段五：測試並儲存檢查點
- [x] 編寫後端 CSV 解析測試（8個測試全部通過）
- [x] 測試各種 CSV 格式（UTF-8、BOM、引號、空白欄位）
- [x] 測試錯誤處理（格式錯誤、空檔案、缺少必要欄位）
- [x] 測試行數限制功能（最多 100 行）
- [x] 儲存檢查點並交付
...
## 修復 CSV 檔案預覽錯誤（2025-01-22）

- [x] 診斷 CSV 預覽對話框顯示「此檔案類型不支援預覽」的錯誤原因
- [x] 檢查 CSVPreviewDialog 元件的程式碼
- [x] 檢查 files.previewCSV API 的實作
- [x] 修復錯誤並測試（在通用預覽對話框中加入 CSV 支援，引導使用者使用專用 CSV 預覽功能）
- [x] 儲存檢查點




## 恢復搜尋關鍵字黃色高亮功能（2025-01-22）
...
### 階段一：CSV 編碼自動偵測功能
- [x] 安裝編碼偵測套件（chardet 或 iconv-lite）
- [x] 建立 encodingDetector.ts 輔助模組
- [x] 實作自動偵測編碼功能（支援 UTF-8、Big5、GBK）
- [x] 實作自動轉換為 UTF-8 功能
- [x] 更新 csvPreview.ts，整合編碼偵測
- [x] 測試各種編碼的 CSV 檔案

### 階段二：CSV 格式驗證功能
- [x] 建立 csvValidator.ts 驗證模組
- [x] 實作必要欄位檢測功能
- [x] 實作資料類型驗證功能（數字、日期、郵件等）
- [x] 實作詳細錯誤訊息生成
- [x] 在上傳時自動執行驗證
- [x] 顯示驗證結果和錯誤提示
- [x] 測試各種格式錯誤的 CSV 檔案
...
### 階段三：CSV 預覽排序和篩選功能
- [x] 在 CSVPreviewDialog 元件新增欄位排序功能
- [x] 實作升序/降序排序邏輯
- [x] 新增資料篩選輸入框
- [x] 實作搜尋特定內容功能
- [x] 新增篩選結果高亮顯示
